...
3. Register — if SMTP isn't configured, email verification will be skipped/best-effort (see [Optional Configuration](#optional-configuration))
...
All of the following are set in the root `.env` file and take effect on the next `docker compose up -d`.
...
```
For Gmail: enable 2FA and generate an "App Password" to use as `SMTP_PASS`.
...
- Set up email notifications so you get alerted when a monitor goes down
