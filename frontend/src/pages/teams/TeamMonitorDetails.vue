<template>
  <div class="p-6 max-w-7xl mx-auto">
    <div class="flex justify-between items-center mb-8">
      <h1 class="text-3xl font-bold text-brand">Team Monitor Detail</h1>
    ...
    <div v-else>
      ...
      <div class="bg-white rounded-lg shadow-md p-6 mb-8">
        ...
        <div v-if="!monitor.active || monitor.is_paused" class="flex flex-col items-center justify-center h-60 text-gray-500">
...
const currentUserRole = computed(() => {
  ...
  const member = team.value.members?.find((m: any) => m.user_id === teamsStore.currentUserId);
...
async function triggerCheck() {
  try {
    ...
    const res = await fetch(`/api/teams/${teamId.value}/monitors/${monitorId.value}/check-now`, {
      method: 'POST',
...
async function sendTestEmail() {
  ...
  try {
    const token = localStorage.getItem('token');
    const res = await fetch(`/api/teams/${teamId.value}/monitors/${monitorId.value}/send-test-email`, {
      method: 'POST',
      headers: token ? { Authorization: `Bearer ${token}` } : {}
    });
    if (!res.ok) {
...
async function fetchMonitorDetail() {
  ...
  try {
    const token = localStorage.getItem('token');
    const res = await fetch(`/api/teams/${teamId.value}/monitors/${monitorId.value}`, { 
      headers: token ? { Authorization: `Bearer ${token}` } : {} 
    });
    ...
    const logsRes = await fetch(`/api/teams/${teamId.value}/monitors/${monitorId.value}/logs?range=${selectedRange.value}`, { 
      headers: token ? { Authorization: `Bearer ${token}` } : {} 
    });
    if (!logsRes.ok) throw new Error('Failed to fetch logs');
    const logsResponse = await logsRes.json();
    
    console.log('Fetched logsResponse:', logsResponse);
    // Extract data from the wrapped response
    ...
    if (logsData.length > 0 && (!monitor.value.lastChecked || !monitor.value.status)) {
      const latestLog = logsData[logsData.length - 1];
      if (!monitor.value.status || monitor.value.status === 'unknown') {
        monitor.value.status = latestLog.status;
      }
