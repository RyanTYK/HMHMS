    ...
    <div class="modal share-modal">
      <div class="modal-header">
        <div class="modal-title-section">
          <h2>Share Multiple Monitors</h2>
          <p class="modal-subtitle">{{ monitorIds.length }} monitors selected</p>
        </div>
      ...
      <div class="modal-body">
        <div class="form-group">
          ...
          <p class="form-hint">All {{ monitorIds.length }} monitors will be shared with this user. When accepted, they will be added to their dashboard.</p>
        </div>
      ...
      <div class="modal-footer">
        ...
        <button class="btn btn-primary" @click="handleBulkShare" :disabled="!isFormValid || sharing">
          {{ sharing ? 'Sharing...' : `Share ${monitorIds.length} Monitors` }}
        </button>
      </div>
    </div>
  </div>
...
async function handleBulkShare() {
  ...
  try {
    ...
    for (const monitorId of props.monitorIds) {
      try {
        await sharesStore.shareMonitor(monitorId, { email: shareEmail.value }, ShareRole.VIEWER);
        successCount++;
      } catch (error) {
        failCount++;
        console.error(`Failed to share monitor ${monitorId}:`, error);
      }
    }

    if (successCount > 0) {
      addToast(`${successCount} monitor(s) shared with ${shareEmail.value}`, 'success');
    }
    if (failCount > 0) {
      addToast(`${failCount} monitor(s) failed to share`, 'error');
    }

    emit('shared');
    emit('close');
  } catch (error: any) {
    errorMessage.value = error.message || 'Failed to share monitors';
